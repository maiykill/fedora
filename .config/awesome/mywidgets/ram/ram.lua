...
local function get_ram_usage()
	...
	local mem_free = meminfo:match("MemFree:%s*(%d+)%s*kB")
	local buffers = meminfo:match("Buffers:%s*(%d+)%s*kB")
	local cached = meminfo:match("Cached:%s*(%d+)%s*kB")
	local slab_reclaimable = meminfo:match("SReclaimable:%s*(%d+)%s*kB")

	if not (mem_total and mem_free and buffers and cached) then
		return nil
	end

	mem_total = tonumber(mem_total)
	mem_free = tonumber(mem_free)
	buffers = tonumber(buffers)
	cached = tonumber(cached)
	slab_reclaimable = tonumber(slab_reclaimable) or 0

	-- Calculate used memory (similar to 'free' command)
	local mem_used = mem_total - mem_free - buffers - cached - slab_reclaimable
